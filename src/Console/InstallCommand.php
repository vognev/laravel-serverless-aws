...
use Laravel\Serverless\Aws\Helper;
...
class InstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $serverless_yml_path = base_path('serverless.yml');
        if (! File::exists($serverless_yml_path)) {
            File::copy(Helper::package_path('templates', 'serverless.yml'),
                       $serverless_yml_path);
        }

        $compose_path = base_path('docker-compose.serverless.yml');
        if (! File::exists($compose_path)) {
            File::copy(Helper::package_path('templates', 'docker-compose.serverless.yml'),
                      $compose_path);
        }
    }
